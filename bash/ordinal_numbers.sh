...
ordinal_numbers_to_file() {
    ...
    declare -a short_ordinals=(0th)
    declare -a long_ordinals=(Zeroth)
    ...
    for entry in "${entries[@]}"; do
        ...
        short_ordinals[num]=$short
        long_ordinals[num]=$long
    done

    test ${#short_ordinals[*]} == ${#long_ordinals[*]} || echo Error!
    test ${#short_ordinals[*]} == ${#long_ordinals[*]} || return 1

    echo "${short_ordinals[@]}" > "$output_file"
    echo "${long_ordinals[@]}" >> "$output_file"
